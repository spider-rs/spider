...
```toml
[dependencies]
spider = { version = "1.31.0", features = ["regex", "ua_generator"] }
```
...
1. `js`: Enables javascript parsing links created with the dom (Alpha).
...
```toml
[dependencies]
spider = { version = "1.31.0", features = ["regex"] }
```
...
```toml
[dependencies]
spider = { version = "1.31.0", features = ["control"] }
```
...
Move processing to a worker, drastically increases performance even if worker is on the same machine due to efficient runtime split IO.

```toml
[dependencies]
spider = { version = "1.31.0", features = ["decentralized"] }
```
