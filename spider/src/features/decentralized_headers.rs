...
pub const WORKER_PROXY_HEADER_PREFIX: &'static str =
    "zz-spider-r--";
...
const PROXIED_ORIGINAL_STATUS: HeaderName = HeaderName::from_static(
    const_format::concatcp!(WORKER_PROXY_HEADER_PREFIX, "status-code")
);
...
fn set_prefix(name: impl AsRef<str>) -> HeaderName {
    ...
    let mut new_value = String::with_capacity(
        WORKER_PROXY_HEADER_PREFIX.len() + key_value.len()
    );
    ...
    HeaderName::try_from(new_value).unwrap()
}
...
pub fn strip_prefix(name: impl AsRef<str>) -> Option<HeaderName> {
    name.as_ref()
        .strip_prefix(WORKER_PROXY_HEADER_PREFIX)
        .map(|stripped| HeaderName::from_str(stripped).unwrap())
}

/// A helper function to check if [name] starts with a [WORKER_PROXY_HEADER_PREFIX].
pub fn has_prefix(name: impl AsRef<str>) -> bool {
    name.as_ref()
        .starts_with(WORKER_PROXY_HEADER_PREFIX)
}
...
impl<T> Extend<(Option<HeaderName>, T)> for WorkerProxyHeaderBuilder<T> {
    fn extend<I: IntoIterator<Item=(Option<HeaderName>, T)>>(&mut self, iter: I) {
        for value in iter.into_iter() {
            self.headers.push((value.0.expect("expected a header name, but got None"), value.1))
        }
    }
}
...
pub fn extend_with_proxy_headers<T, I: IntoIterator<Item = (Option<HeaderName>, T)>>(target: &mut HeaderMap<T>, proxied_headers: I) {
    target.extend(
        proxied_headers.into_iter().map(|(key, value)| (
            key.map(|key_value| set_prefix(&key_value)),
            value
        ))
    )
}
...
pub fn split_proxy_headers<T, I: IntoIterator<Item=(HeaderName, T)>>(header: I) -> HeaderSplit<T> {
    let (a, b) = header
        .into_iter()
        .partition_map(|(k, v)|
...
pub fn extract_proxy_headers<T: Clone>(src: &HeaderMap<T>) -> HeaderMap<T> {
    HeaderMap::from_iter(
        src.into_iter().filter_map(|(key, value)|{
            Some((strip_prefix(key)?, value.clone()))
        })
    )
}
...
pub mod tests {
    ...
    use super::{set_prefix, extract_proxy_headers, PROXIED_ORIGINAL_STATUS, STATUS_CODE_HEADER_FIELD, WorkerProxyHeaderBuilder};
