//! `GEMINI_API_KEY=$MYAPI_KEY cargo run --example gemini --features "spider/sync spider/gemini"`
extern crate spider;

use std::time::Duration;

use spider::configuration::{GeminiConfigs, WaitForIdleNetwork};
...
async fn main() {
    ...
    let mut gemini_config = GeminiConfigs::default();
    let prompt_url_map = HashMap::from([
        (
            CaseInsensitiveString::new(website_url),
            Box::new(GeminiConfigs::new(
                "gemini-flash-latest",
                "Search for Movies",
                500,
            )),
    ...
    gemini_config.prompt_url_map = Some(Box::new(prompt_url_map));
    ...
    let mut website: Website = Website::new(website_url)
        ...
        .with_gemini(Some(gemini_config))
        .build()
        .unwrap();
