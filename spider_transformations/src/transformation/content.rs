...
pub struct TransformConfig {
    /// The Unicode encoding to use like UTF-8 etc.
    pub encoding: Option<String>,
    /// Readability mode.
    pub readability: bool,
    /// The root selector for the text element.
...
fn get_html(c: &TransformConfig, res: &Page) -> String {
    match c.encoding {
...
pub fn transform_content(res: &Page, c: &TransformConfig) -> String {
    ...
    match return_format {
        ...
        ReturnFormat::Html2Text => match c.encoding {
            Some(ref encoding) => {
                let b = res.get_html_encoded(encoding);
        ...
        ReturnFormat::Text => {
            ...
            let d = if c.root_selector.is_some() {
                let selector = &match c.root_selector {
