...
fn summarize_response_usage(value: &serde_json::Value) -> String {
    ...
    let has_metadata = first.get("metadata").is_some();

    format!(
        "status={}, duration_ms={}, total_cost={}, metadata_present={}",
        status, duration_ms, total_cost, has_metadata
    )
}

#[tokio::test]
async fn live_spider_cloud_search_and_scrape_tools() -> Result<(), Box<dyn std::error::Error>> {
    if !run_live_tests() {
        eprintln!("Skipping Spider Cloud live test (RUN_LIVE_TESTS not enabled).");
        return Ok(());
    }
    ...
    assert!(agent.has_custom_tool("spider_cloud_search"));
    assert!(agent.has_custom_tool("spider_cloud_scrape"));
    ...
    eprintln!(
        "Spider Cloud scrape usage: {}",
        summarize_response_usage(&scrape_json)
    );

    let usage = agent.usage();
    eprintln!(
        "Agent usage snapshot: search_tool_calls={}, scrape_tool_calls={}, total_custom_tool_calls={}",
    ...
    assert!(
        usage.total_custom_tool_calls() >= 2,
        "expected at least 2 custom tool calls"
    );
