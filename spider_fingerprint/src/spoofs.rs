...
/// Plugin extension.
pub const PLUGIN_AND_MIMETYPE_SPOOF: &str = r#"(()=>{const mimeData=[{type:'application/pdf',suffixes:'pdf',description:'Portable Document Format'},{type:'text/pdf',suffixes:'pdf',description:'Portable Document Format'}];const pluginNames=['PDF Viewer','Chrome PDF Viewer','Chromium PDF Viewer','Microsoft Edge PDF Viewer','WebKit built-in PDF'];const plugins=[];const mimes=[];pluginNames.forEach(name=>{const plugin=Object.create(Plugin.prototype);Object.defineProperties(plugin,{name:{value:name},description:{value:'Portable Document Format'},filename:{value:'internal-pdf-viewer'},length:{value:mimeData.length}});mimeData.forEach((mime,i)=>{const mimeType=Object.create(MimeType.prototype);Object.defineProperties(mimeType,{type:{value:mime.type},suffixes:{value:mime.suffixes},description:{value:mime.description},enabledPlugin:{value:plugin}});plugin[i]=mimeType;mimes.push(mimeType)});plugins.push(plugin)});const pluginArray=Object.create(PluginArray.prototype);plugins.forEach((p,i)=>{Object.defineProperty(pluginArray,i,{value:p,enumerable:true});Object.defineProperty(pluginArray,p.name,{value:p,enumerable:false})});Object.defineProperty(pluginArray,'length',{value:plugins.length,enumerable:false});Object.defineProperty(pluginArray,'item',{value:i=>pluginArray[i]||null});Object.defineProperty(pluginArray,'namedItem',{value:n=>pluginArray[n]||null});Object.defineProperty(pluginArray,'toJSON',{value:()=>[...plugins],enumerable:false});const mimeArray=Object.create(MimeTypeArray.prototype);mimes.forEach((m,i)=>{Object.defineProperty(mimeArray,i,{value:m,enumerable:true});Object.defineProperty(mimeArray,m.type,{value:m,enumerable:false})});Object.defineProperty(mimeArray,'length',{value:mimes.length,enumerable:false});Object.defineProperty(mimeArray,'item',{value:i=>mimeArray[i]||null});Object.defineProperty(mimeArray,'namedItem',{value:n=>mimeArray[n]||null});const nativeGetter=(v,n)=>{const f=()=>v;Object.defineProperty(f,'toString',{value:()=>`function get ${n}() { [native code] }`});return f};Object.defineProperties(Navigator.prototype,{plugins:{get:nativeGetter(pluginArray,'plugins')},mimeTypes:{get:nativeGetter(mimeArray,'mimeTypes')}})})();"#;
