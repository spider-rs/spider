...
pub const HIDE_CHROME: &str = r#"(()=>{try{const c=navigator.userAgentData?.brands?.some(b=>b.brand.includes("Chromium")||b.brand.includes("Chrome"))||navigator.userAgent.includes("Chrome")&&!!window.chrome;if(!c)return;if(!window.chrome)window.chrome={};const val={InstallState:{DISABLED:"disabled",INSTALLED:"installed",NOT_INSTALLED:"not_installed"},RunningState:{CANNOT_RUN:"cannot_run",READY_TO_RUN:"ready_to_run",RUNNING:"running"},get isInstalled(){return false},getDetails:function getDetails(){if(arguments.length)throw new TypeError("Error in invocation of app.getDetails()");return null},getIsInstalled:function getIsInstalled(){if(arguments.length)throw new TypeError("Error in invocation of app.getIsInstalled()");return false},runningState:function runningState(){if(arguments.length)throw new TypeError("Error in invocation of app.runningState()");return'cannot_run'}};Object.defineProperty(window.chrome,'app',{get:function(){return val},set:function(){val=arguments[0]},enumerable:true,configurable:true});if(window.performance&&performance.timing){var t=performance.timing;window.chrome.csi=function(){return{onloadT:t.domContentLoadedEventEnd,startE:t.navigationStart,pageT:Date.now()-t.navigationStart,tran:15}}}if(window.PerformancePaintTiming){function nav(){return performance.getEntriesByType('navigation')[0]||{nextHopProtocol:'h2',type:'other'}}var p={connectionInfo:nav().nextHopProtocol,npnNegotiatedProtocol:['h2','hq'].indexOf(nav().nextHopProtocol)>-1?nav().nextHopProtocol:'unknown',navigationType:nav().type,wasAlternateProtocolAvailable:false,wasFetchedViaSpdy:['h2','hq'].indexOf(nav().nextHopProtocol)>-1,wasNpnNegotiated:['h2','hq'].indexOf(nav().nextHopProtocol)>-1};var u={firstPaintAfterLoadTime:0,requestTime:performance.timing.navigationStart/1e3,startLoadTime:performance.timing.navigationStart/1e3,commitLoadTime:performance.timing.responseStart/1e3,finishDocumentLoadTime:performance.timing.domContentLoadedEventEnd/1e3,finishLoadTime:performance.timing.loadEventEnd/1e3,firstPaintTime:function(){var e=performance.getEntriesByType('paint')[0]||{startTime:performance.timing.loadEventEnd/1e3};return((e.startTime+performance.timeOrigin)/1e3).toFixed(3)}};window.chrome.loadTimes=function(){return{connectionInfo:p.connectionInfo,npnNegotiatedProtocol:p.npnNegotiatedProtocol,navigationType:p.navigationType,wasAlternateProtocolAvailable:p.wasAlternateProtocolAvailable,wasFetchedViaSpdy:p.wasFetchedViaSpdy,wasNpnNegotiated:p.wasNpnNegotiated,firstPaintAfterLoadTime:u.firstPaintAfterLoadTime,requestTime:u.requestTime,startLoadTime:u.startLoadTime,commitLoadTime:u.commitLoadTime,finishDocumentLoadTime:u.finishDocumentLoadTime,finishLoadTime:u.finishLoadTime,firstPaintTime:u.firstPaintTime()}}}}catch(e){}})();"#;
